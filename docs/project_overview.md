...
4. 映像が表示されたら `領域作成 +` で推論領域を追加
5. 赤枠をドラッグで移動、右下のハンドルでサイズ調整
6. 右側のリストに領域ごとの推論結果が表示される（削除は `−`）
7. リストはドラッグで並べ替え、領域名と色を編集できる

## 推論領域と結果の仕様
- 推論領域は画面上の赤枠矩形として表示される。
- 領域内の検出結果は、検出ボックスの左端座標が小さい順に並べて文字列化する。
  - 例: `"0","1"` → `01`
- 検出ボックスがほぼ重なる場合は、確信度の高い方のみを採用する（IoU閾値あり）。
- フロント内部はピクセル座標で管理し、送信時に百分率へ変換する。
- 領域はブラウザの `localStorage` に保存され、再読み込み後も復元される。
...
## WebSocketメッセージ仕様（概要）
### クライアント → サーバ
- 開始: `{"type":"start","source":"<url or device:0>","model":"model_name.pt","regions":[...]}`
